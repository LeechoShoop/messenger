...
impl PrimusDHT {
    ...
    pub async fn insert<P: NodePinger>(&self, nr: PrimusNR, pinger: &P) {
        // Hot path: runs once per inbound peer record. Same reasoning as
        // KademliaEngine::handle_new_peer (lib.rs) — ML-DSA-87 verify goes
        // through spawn_blocking rather than inline on the async executor
        // thread. Relies on the runtime's enlarged blocking-pool stack
        // (tokio::runtime::Builder::thread_stack_size(16 MiB), see main.rs),
        // not a per-call std::thread::Builder.
        let nr_c = nr.clone();
