...
//   peer Node Records is delegated to PrimusNR::verify() (peer.rs, local to
//   this crate). The direct ml-dsa usage here is correct because
...
/// Handshake payload embedded in Noise messages 2 and 3: the sender's
/// self-signed Node Record plus a signature binding it to the peer's
/// ephemeral Noise key. This is what prevents identity-misbinding attacks
/// (see module header). Local to this crate now — previously came from
/// primus-types.
    ...
    S: AsyncRead + AsyncWrite + Unpin,
{
    ...
    pub async fn handshake_initiator(
        ...
        // Verify the responder's Node Record self-signature. This runs on
        // every handshake and was previously called inline with no
        // protection at all (not even the old nested-thread pattern covered
        // it) — spawn_blocking here as well, same reasoning as the
        // ephemeral-key checks below.
        let nr_c = payload.nr.clone();
        let nr_verified = tokio::task::spawn_blocking(move || nr_c.verify())
            .await
            .unwrap_or(false);
        if !nr_verified {
            return Err(anyhow!(
        ...
        // NOTE: no nested std::thread::Builder here anymore — the runtime's
        // blocking-pool threads now carry the enlarged stack configured via
        // tokio::runtime::Builder::thread_stack_size(16 MiB) (see main.rs),
        // so spawn_blocking alone is sufficient to keep ML-DSA-87 off the
        // async executor without needing a second OS thread per call.
        let initiator_e_c = initiator_e.clone();
        let verify_result =
            tokio::task::spawn_blocking(move || responder_vk.verify(&initiator_e_c, &sig))
                .await
        ...
        let initiator_sig =
            tokio::task::spawn_blocking(move || signer.sign(&responder_e_c).to_bytes().to_vec())
                .await
                .expect("spawn_blocking failed");
        ...
        // `payload` (message 2) holds the responder's self-signed, already-
        // verified Node Record — captured here before it goes out of scope,
        // so callers no longer have to re-derive/re-fetch it separately.
        let peer_nr = payload.nr.clone();

        let noise = handshake.into_stateless_transport_mode()?;
        Ok((Self::new(inner, noise), peer_nr))
    }
    ...
    pub async fn handshake_responder(
        ...
        let initiator_e_c = initiator_e.clone();
        let responder_sig =
            tokio::task::spawn_blocking(move || signer.sign(&initiator_e_c).to_bytes().to_vec())
                .await
                .expect("spawn_blocking failed");
        ...
        // Verify the initiator's Node Record self-signature. Same fix as the
        // responder side above: previously called inline with no protection.
        let nr_c = payload.nr.clone();
        let nr_verified = tokio::task::spawn_blocking(move || nr_c.verify())
            .await
            .unwrap_or(false);
        ...
        let responder_e_c = responder_e.clone();
        let verify_result =
            tokio::task::spawn_blocking(move || initiator_vk.verify(&responder_e_c, &sig))
                .await
                .expect("spawn_blocking failed");

        verify_result.map_err(|e| anyhow!("Initiator identity binding check failed: {e}"))?;

        // `payload` (message 3) holds the initiator's self-signed,
        // already-verified Node Record — captured here before it goes out
        // of scope, so callers no longer have to re-derive/re-fetch it.
        let peer_nr = payload.nr.clone();
